#!/usr/bin/env Rscript
### Author: Yang Eric Li (yal054@ucsd.edu)
...
parser$add_argument("-i", "--input", required=TRUE, help="list of summit bed, two columns: label, summits.bed")
...
parser$add_argument("--size", default = "500", help="extend peak to fixed size (default 500bp)")
...
# extend summit to desired size bp
...
peak.list = lapply(seq(file.lst), function(i){
  ...
  p.gr <- extendSummit(p.gr, size=as.integer(ssize))
